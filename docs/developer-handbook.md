...
- elFinder's own CSRF protection is enforced on every write command: an `X-elFinder-CSRF` header carrying a `random_bytes(32)` token, validated with `hash_equals()`, with a 900s TTL. Verified end-to-end: a write request with the header stripped or wrong returns `403` with `{"error":["errPerm","Invalid request. Please reload."],"csrfReload":true}`; with the correct token it returns `200`. The client already implements the full handshake — `public/be-assets/plugins/elFinder/js/elfinder.full.js` (`csrfHeaderName`, `customHeaders`) attaches the token on both the jQuery request path and the raw-XHR upload path, and auto-recovers via the `csrfReload` flag. `MediaConfig::$csrfExcept` deliberately still excepts this endpoint from CI4's *own* CSRF filter — not because elFinder's CSRF is disabled, but because `elFinderConnector::output()` always ends in `exit()` (`elFinderConnector.class.php:432,562,565`), so CI4's after-filters — including the one that rotates the CI4 token under `security.regenerate = true` — never run on this route; removing the except would desynchronize the CI4 token after the very first write. Known limit: `editor` is not one of elFinder's own `$csrfProtectedCmds` (13 commands in 2.1.70, `elFinderConnector.class.php:79-93`).
