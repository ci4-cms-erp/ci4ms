...
class CreateCategoriesLangsTable extends Migration
{
    public function up()
    {
        // fieldExists() results are cached on the connection and go stale if the
        // schema changes within this process (migrate:refresh / rollback+migrate).
        // Reset first so the guard doesn't lie.
    ...
    public function down()
    {
        ...
        if (!$this->db->fieldExists('title', 'categories')) {
            try {
                ...
                // Don't error if the column already exists
            }
        }
