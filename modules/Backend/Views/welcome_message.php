...
<style>
    ...
    .btn-widget-hide:hover { opacity: 1; color: #fff; }
    ...
    .widget-toggle-item:hover { background: #f8f9fa; }
    ...
    $(document).on('click', '.btn-widget-hide', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res[csrfName]) csrfHash = res[csrfName];
                if (res.status === 'success') {
                    $widgetEl.animate({ opacity: 0 }, 300, function () { $(this).remove(); });
                    toastr && toastr.info(res.message || 'Widget gizlendi');
                }
            },
            error: function (xhr) { console.error('Toggle hatası:', xhr.responseText); }
        });
    });
