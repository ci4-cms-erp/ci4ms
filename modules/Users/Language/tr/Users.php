<?php return [
    'usersCrud' => 'Kullanıcı İşlemleri',
    'users' => 'Kullanıcılar',
    'addUser' => 'Kullanıcı Ekleme',
    'permGroupList' => 'Yetki Grup Listesi',
    'permGroupCreate' => 'Yetki Grubu Oluştur',
    'permUpdate' => 'yetkilerini güncelle',
    'permGroupName' => 'Yetki Grubu Adı',
    'perms' => 'Yetkilendirmeler',
    'groupList' => 'Yetkiler',
    'addGroupPerms' => 'Grup Yetkisi Ekleme',
    'profile' => 'Profil',
    'logout'=>'Çıkış Yap',
    'updateGroupPerms' => 'Grup Yetkisi Güncelleme',
    'specialAuthUser' => 'Kullanıcıya özel yetki verme',
    'updateUser' => 'Kullanıcı Güncelleme',
    'removeFromBlackList' => 'Kara Listeden Çıkar ve Aktivasyon Maili Gönder',
    'whyAddedBlakList' => 'Kara listeye eklenme sebebi',
    'blackList' => 'Kara Liste',
    'spacialAuth' => 'Özel Yetki',
    'resetPassword' => '<i class="fas fa-key"></i> Şifre Sıfırla',
    'inBlackList' => 'Kara Listede',
    'authority' => 'Yetkisi',
    'userList' => 'Kullanıcı Listesi',
    'searchPlaceholder' => 'Kullanıcı ara...',
    'removeFromBlacklistBtn' => 'Listeden Çıkar',
    'addToBlacklistBtn' => 'Kara Listeye Al',
    'removeFromBlacklistTitle' => 'Kara Listeden Çıkar',
    'addToBlacklistTitle' => 'Kara Listeye Ekle',
    'totalUsers' => 'Toplam Kullanıcı',
    'bannedUsers' => 'Kara Liste',
    'firstname' => 'Ad',
    'surname' => 'Soyad',
    'email' => 'E-posta Adresi',
    'password' => 'Şifre',
    'userGroups' => 'Yetki Grupları',
    'alreadyTakenEmail' => 'Daha önce bu mail adresi başka bir kullanıcı tarafından alınmıştır lütfen bilgilerinizi kontrol ediniz.',
    'addedToBlacklist' => 'Üyelik karalisteye eklendi.',
    'couldNotAddToBlacklist' => 'Üyelik karalisteye eklenemedi.',
    'blacklistProcess' => 'Kara Liste İşlemi',
    'currentNote' => 'Mevcut Not',
    'removedFromBlacklist' => '{0} e-mail adresli üyelik karalisteden çıkarıldı.',
    'couldNotRemoveFromBlacklist' => 'Üyelik karalisteden çıkarılamadı.',
    'passwordResetStep' => '{0} e-mail adresli üyelik şifre sıfırlama adımında.',
    'passwordResetSuccess' => '{0} e-mail adresli üyelik şifresi sıfırlandı.',
    'passwordResetError' => '{0} e-mail adresli üyelik şifresi sıfırlanamadı.',
    'terminateOtherSessions' => 'Diğer Oturumları Sonlandır',
    'terminateSession' => 'Oturumu Sonlandır',
    'activeSessions' => 'Aktif Oturumlar',
    'accountAccessDevices' => 'Hesabınıza erişim sağlayan cihazlar',
    'terminateOthers' => 'Diğerlerini Kapat',
    'terminateOtherSessionsConfirm' => 'Mevcut oturum hariç tüm oturumlar kapatılacak. Emin misiniz?',
    'noActiveSessions' => 'Aktif oturum bulunamadı.',
    'thisDevice' => 'Bu cihaz',
    'lastActivity' => 'Son aktivite:',
    'currentSession' => 'Mevcut Oturum',
    'terminateConfirm' => 'Bu oturumu kapatmak istediğinizden emin misiniz?',
    'sessionHistory' => 'Oturum Geçmişi',
    'opened' => 'Açılış',
    'closed' => 'Kapanış',
    'closedBadge' => 'Kapalı',
    'sessionTerminated' => 'Oturum başarıyla sonlandırıldı.',
    'sessionTerminateError' => 'Oturum kapatılamadı veya bulunamadı.',
    'sessionTerminatedCount' => '{0} oturum başarıyla sonlandırıldı.',
    'invalidRequest' => 'Geçersiz istek.',
    'currentSessionTerminated' => 'Mevcut oturumunuz kapatıldı.',
    'avatarSizeExceeded' => 'Dosya boyutu 2MB\'dan büyük olamaz.',
    'last10PastSessions' => 'Sadece son 10 geçmiş oturum listelenmektedir.',
    'cannotDeleteSuperadmin' => 'Superadmin kullanıcıları silinemez.',
    'currentPassword' => 'Mevcut Şifre',
    'currentPasswordWrong' => 'Girdiğiniz mevcut şifre hatalı.',
    'superadminGroupProtected' => 'Superadmin grubu yeniden adlandırılamaz veya değiştirilemez.',
    'groupNameConflict' => 'Bu grup adı zaten kullanılıyor.',
    'cannotEditOwnGroupName' => 'Şu anda içinde bulunduğunuz grubun adını değiştiremezsiniz.',
    'permsExceedOwnGrant' => 'Kendinizde olmayan bir izni başka bir gruba veremezsiniz.',
    'permsMalformed' => 'Gönderilen izin verisi hatalı biçimde.',
    'auditPermsDelegationRejected' => '{0} kullanıcısı, "{1}" üzerine kendisinde olmayan izinler vermeye çalıştı.',
    'auditGroupCreated' => '"{1}" yetki grubu {0} tarafından oluşturuldu.',
    'auditGroupUpdated' => '"{1}" yetki grubu {0} tarafından güncellendi.',
    'auditUserPermsUpdated' => '{0}, {1} kullanıcısının özel izinlerini güncelledi.',
    'auditUserUpdated' => '{1} kullanıcısı {0} tarafından güncellendi (gruplar: {2}).',
    'auditUserUpdatedWithPasswordReset' => '{1} kullanıcısı {0} tarafından güncellendi (gruplar: {2}) ve şifresi sıfırlandı.',
    'cannotEditOwnPermissions' => 'Kendi izinlerinizi düzenleyemezsiniz.',
    'cannotEditOwnAccount' => 'Kendi hesabınızı bu ekrandan düzenleyemezsiniz; profil sayfanızı kullanın.',
    'groupExceedsOwnGrant' => 'Kendinizde olmayan yetkilere sahip bir grubu atayamazsınız.',
    'cannotResetPeerPassword' => 'Superadmin yetkiniz olmadan bu ekrandan başka bir kullanıcının şifresini belirleyemezsiniz.',
    'profileIMG' => 'Profil Resmi',
];
