...
<section class="content">
    ...
    <div class="card card-outline card-shl">
        ...
        <div class="card-body">
            ...
            <form action="<?= route_to('user_perms', $userInfos->id) ?>" method="post" class="form-row">
                <?= csrf_field() ?>
                                    ...
                                    foreach ($module->pages as $key => $page):
                                        ...
                                        <div class="page-item col-md-4 border" data-page-id="<?= $page->id ?>" data-status="inactive" data-content="<?= htmlspecialchars($page->description) ?>">
                                            ...
                                            <label class="toggle-switch page-toggle">
                                                <input type="checkbox" name="perms[<?= $page->id ?>][roles]" <?php foreach ($perms as $p) if ($p->page_id == $page->id && ($p->create_r || $p->read_r || $p->update_r || $p->delete_r)) {
                                                                                                                    echo 'checked';
                                                                                                                    break;
                                                                                                                } ?> value="<?= $page->typeOfPermissions ?>">
