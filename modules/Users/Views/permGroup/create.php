...
<section class="content">
    ...
    <div class="card card-outline shadow-sm">
        ...
        <div class="card-body">
            <form action="<?php echo route_to('group_create') ?>" method="post">
                <?php echo csrf_field() ?>
                <div class="row">
                    <div class="col-md-6">
                        <label for=""><?php echo lang('Users.permGroupName') ?></label>
                        <input type="text" class="form-control" name="groupName" value="<?php echo old('groupName') ?>" required>
                    </div>
                    <div class="col-md-6">
                        <label for="">Seflink</label>
                        <input type="text" class="form-control" name="seflink" value="<?php echo old('seflink') ?>" required>
                    </div>
                    <div class="col-md-12 mt-2">
                        <label for=""><?php echo lang('Backend.content') ?></label>
                        <textarea name="description" cols="30" rows="5" class="form-control" required><?php echo old('description') ?></textarea>
                    </div>
                </div>
                ...
                <div id="modulesContainer">
                    ...
                    ?>
                        <div class="m-module-card module-card"
                                        ...
                                        <?php foreach ($module->pages as $page): ?>
                                            ...
                                            <tr class="page-item"
                                                ...
                                                <td style="text-align:center">
                                                    <span class="m-status-pill m-status-inactive status-badge"><?php echo lang('Backend.passive') ?></span>
                                                    <label class="toggle-switch page-toggle">
                                                        <input type="checkbox" name="perms[<?php echo $page->id ?>][roles]" value="<?php echo $page->typeOfPermissions ?>">
                                                        <span class="toggle-slider"></span>
                                                    </label>
                                                </td>
                                            </tr>
    ...
    (function () {
        ...
        function syncModuleToggle(moduleCard) {
            ...
            if (checkedCnt === 0) {
                ...
                $card.data('status', 'inactive').attr('data-status', 'inactive');
            } else if (checkedCnt === total) {
                toggleEl.checked       = true;
                toggleEl.indeterminate = false;
                $card.data('status', 'active').attr('data-status', 'active');
            } else {
                toggleEl.checked       = true;
        ...
        $('.module-toggle-input').on('change', function (e) {
            ...
            if (this.checked) {
                ...
                moduleCard.attr('data-status', 'active');
            } else {
                moduleCard.find('.page-toggle input').each(function () {
                ...
                moduleCard.attr('data-status', 'inactive');
            }
        });
