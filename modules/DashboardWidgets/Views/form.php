            ...
            <div class="card card-outline card-primary shadow-sm">
                ...
                <form id="widgetForm" action="<?php echo $formAction ?>" method="post">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label><?php echo lang('DashboardWidgets.allowedGroups') ?></label>
                            <select class="form-control select2" name="allowed_groups[]" multiple>
                                <?php
                                $selectedGroups = ($isEdit && !empty($widget->allowed_groups)) ? json_decode($widget->allowed_groups, true) : [];
                                foreach ($authGroups as $g): ?>
                                    <option value="<?php echo esc($g->group) ?>" <?php echo in_array($g->group, (array)$selectedGroups) ? 'selected' : '' ?>>
                                        <?php echo esc($g->group) . (!empty($g->description) ? ' (' . esc($g->description) . ')' : '') ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <small class="form-text text-muted"><?php echo lang('DashboardWidgets.allowedGroupsPlaceholder') ?>. <br><?php echo lang('DashboardWidgets.superAdminNote') ?></small>
                        </div>
